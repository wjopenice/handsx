/**
 * Created by adminn on 2018/4/12.
 */
"use strict";
layui.use(['layer', 'jquery', 'table', 'form', 'upload', 'laydate', 'element'], function () {
    ...
    form.on('submit(search)',function(data){
        table.reload('test', {
            url: '/index/Hwith/getWetchWith.html',
            where: data.field
        });
        return false;
    });
    ...
    $(document).on('click','#accounts',function(){
        var url =  '/index/hwith/wetch_sett.html';
        $.ajax({
            url : url,
            type : 'post',
            ...
            success:function(data){
                if(data.code == 1){
                    layer.open({
                        ...
                        maxmin: true,
                        title:'申请提现',
                        content: 'index/hwith/applyFor?type=wetch'
                    });
                    return false;
                }
                if(data.code == 0){
    ...
    table.render({
        ...
        ,height: 'full'
        ,even:true
        ,url: '/index/Hwith/getWetchWith.html' //数据接口
        ,page: true //开启分页
        ,limit: 15
        ,width: 1673
        ,cols: [[ //表头

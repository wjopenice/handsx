...
layui.use(['layer', 'jquery', 'table', 'form', 'upload', 'laydate', 'element'], function () {
    ...
    table.render({
        ...
        ,height: 'full'
        ,even:true
        ,url: '/admin/stall/getStall.html' //数据接口
        ,page: true //开启分页
        ,limit:10
        ,cols: [[ //表头
            ...
            ,{field: 'h5_wetch_revenue', title: '提现手续费（H5微信）', width:250, sort: true,align:'center'}
            ,{field: 'h5_alipay_revenue', title: '提现手续费（H5支付宝）', width:250, sort: true,align:'center'}
            ,{field: 'comm_revenue', title: '佣金提现费率', width:250, sort: true,align:'center'}
            ,{field: 'with_cycle', title: '佣金提现周期', width:250, sort: true,align:'center'}
    ...
    table.on('tool(stall)', function(obj){
        ...
        if(obj.event === 'detail'){
            ...
            layer.confirm('真的删除行么', function(index){
                obj.del();
                layer.close(index);
                admin.ajax.post('/admin/stall/delStall.html',{'id': data.id}, function(result){
                    if(result.code == 1){
            ...
            layer.open({
                ...
                maxmin: true,
                title:'档位设置',
                content: '/admin/stall/editStall.html?id='+data.id,
            });
        }
    });
    ...
    form.on('submit(editStall)',function (data) {
        $.ajax({
            type:"post",
            url:'/admin/stall/editStall.html',
            data:data.field,
            dataType:'json',

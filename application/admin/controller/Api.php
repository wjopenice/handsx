...
//                break;
                echo json_encode(["code"=>0,"msg"=>"暂不支持结果查询"]);
    ...
    //兆行订单查询
    public function SuPaySelect($data){
        $logFile = fopen(dirname(__FILE__)."/pay.txt", "a+");
        $pay_data = db('pay_interface')->where("id",$data['pay_source'])->find();

        #支付配置
        $config["mchNo"] = $pay_data['pay_appid'];
        $config["orderCode"] = $data['pay_order_number'];
        $config["ts"] = $this->get_total_millisecond();

        $config["sign"] = $this->SignSuArray($config,$pay_data['pay_appkey']);//签名

        $paramsStr = $this->ToUrlParams($config);
        $url = "http://www.51manqian.com/api/queryOrder";
        $response = $this->request($url, $paramsStr);
        fwrite($logFile, "兆行支付返回-->".$response."\r\n");

        $rspArray = json_decode($response, true);
        //echo "<pre>";
        //print_r($rspArray);exit;
        if($rspArray['code'] == 0){
            if($rspArray['data']['status'] == 2 || $rspArray['data']['status'] == 3){
                $pay_status = 1;
                $order_number = $rspArray['data']['tradeNo'];
                $msg = '支付成功';
                $re = db('promote_deposit')
                    ->where("pay_order_number = '".$rspArray['data']['orderCode']."'")
                    ->update(['pay_status' => $pay_status,"order_number" => $order_number]);
            }else if($rspArray['data']['status'] == 1){
                $msg = '待支付';
            }else{
                $msg = '支付失败';
            }
    ...
    //兆行
    public function SignSuArray(array $array,$appkey){

        ksort($array);
        $blankStr = $this->ToUrlParams($array);
        $blankStr = $blankStr."&token=".$appkey;// 将key放到数组中一起进行排序和组装
